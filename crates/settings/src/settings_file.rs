...
mod tests {
    ...
    async fn test_watch_config_file_reloads_when_parent_dir_is_symlink(cx: &mut TestAppContext) {
        cx.executor().allow_parking();
        let fs = FakeFs::new(cx.background_executor.clone());
        let config_settings_path = PathBuf::from("/root/.config/zen/settings.json");
        let target_settings_path = PathBuf::from("/root/dotfiles/zen/settings.json");
        ...
        fs.create_symlink(
            Path::new("/root/.config/zen"),
            PathBuf::from("/root/dotfiles/zen"),
        )
        .await
        .unwrap();
