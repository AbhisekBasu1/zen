...
#[cfg(test)]
use workspace::{OpenOptions, PathList, SerializedWorkspaceLocation};
    ...
    pub fn parse(request: RawOpenRequest, _cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("zen-cli://") {
                ...
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("zen://ssh") {
                log::info!("ignoring SSH file URL in stripped build: zen://ssh{file}");
            } else if let Some(extension_id) = url.strip_prefix("zen://extension/") {
                ...
                log::info!("ignoring SSH URL in stripped build: {url}");
            } else {
    ...
    for (location, workspace_paths) in grouped_locations {
        ...
        match location {
            ...
            SerializedWorkspaceLocation::Remote(_) => {
                log::info!("ignoring remote workspace open in stripped build");
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::zen::{open_listener::open_local_workspace, tests::init_test};
    use cli::CliResponse;
    use editor::Editor;
    use futures::poll;
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zen://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zen://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["zen://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zen://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Zen to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell Zen to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    fn test_git_clone_url_is_ignored(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "zen://git/clone/?repo=https://github.com/example/project.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
    ...
    fn test_git_clone_url_with_encoding_is_ignored(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "zen://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fexample%2Fproject.git"
                            .into(),
                    ],
                    ..Default::default()
                },
    ...
    async fn test_e2e_no_flags_no_windows_no_prompt(cx: &mut TestAppContext) {
        ...
        let (status, prompt_shown) = run_cli_with_zen_handler(
            cx,
            app_state,
