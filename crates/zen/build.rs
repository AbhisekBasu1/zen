...
fn main() {
    #[cfg(target_os = "linux")]
    {
        ...
        for lib in &dlopened_libs {
            if let Some(libdir) = pkg_config::get_variable(lib, "libdir").ok() {
                ...
                eprintln!("zen build.rs: {lib} not found in pkg-config's path");
            }
        }
    ...
    if cfg!(target_os = "macos") {
        ...
        // Weakly link ReplayKit to ensure Zen can be used on macOS 10.15+.
    ...
    let git_sha = match std::env::var("ZEN_COMMIT_SHA").ok() {
        Some(git_sha) => {
    ...
    if let Some(git_sha) = git_sha {
        println!("cargo:rustc-env=ZEN_COMMIT_SHA={git_sha}");

        if let Some(build_identifier) = option_env!("GITHUB_RUN_NUMBER") {
            println!("cargo:rustc-env=ZEN_BUILD_ID={build_identifier}");
        }
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for ZEN_COMMIT_SHA env var");
        }
    }
...
fn prepare_app_icon_x11() {
    ...
    // name should match include_bytes! call in src/zen.rs
